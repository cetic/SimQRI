...
object SimQRiModel {
  ...
  def createProbesMaps(fm : FactoryModel) : (Map[String, DataSampling],Map[String,List[(Double,List[Double])]]) = {
    ...
    if (fm.ms != null) {
      for {(name, expr) <- fm.ms.rootExpressions} {
        expr match {
          case be:BoolExpr =>
    ...
    if (fm.ms != null) {
      for {(name, expr) <- fm.ms.rootExpressions} {
        expr match {
          ...
          case bhe:BoolHistoryExpr =>
            newHistoryProbesMap += (name -> addListDoubleToList(bhe.history.map(tup => (tup._1, if (tup._2) 1D else 0D)),
                                                                historyProbesMap(name)))
          case dhe:DoubleHistoryExpr =>
            newHistoryProbesMap += (name -> addListDoubleToList(dhe.history, historyProbesMap(name)))
          case _ =>
        }
      }
    }
